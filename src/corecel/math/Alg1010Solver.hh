...
class Alg1010Solver
{
  ...
  public:
    ...
    inine CELER_FUNCTION Alg1010Solver() : Alg1010Solver{default_tol_} {}
  ...
  private:
    ...
    inline CELER_FUNCTION real_type
    calc_err_abcd_cmplx(double a,
                        double b,
                        double c,
                        double d,
                        std::complex<double> aq,
                        std::complex<double> bq,
                        std::complex<double> cq,
                        std::complex<double> dq) const;
    ...
    solve_quadratic(double a, double b, std::complex<double> roots[2]) const;
}

// pow(DBL_MAX,1.0/3.0)/1.618034;
constexpr double Alg1010Solver::CUBIC_RESCAL_FACT = 3.488062113727083e+102;
// pow(DBL_MAX,1.0/4.0)/1.618034;
constexpr double Alg1010Solver::QUART_RESCAL_FACT = 7.156344627944542e+76;
constexpr double Alg1010Solver::MACHEPS
    = std::numeric_limits<double>::epsilon();

double
Alg1010Solver::solve_cubic_analytic_depressed_handle_inf(double b, double c)
{
...
double Alg1010Solver::solve_cubic_analytic_depressed(double b, double c)
{
    /* find analytically the dominant root of a depressed cubic x^3+b*x+c
...
double
Alg1010Solver::calc_phi0(double a, double b, double c, double d, int scaled)
{
    ...
    double h = (8 * dq + hh - 2 * gg) * bq / 3 - cq * cq - dq * aq * aq; /* eq.
                                                                            (86)
                                                                          */
    ...
    if (std::isnan(rmax) || std::isinf(rmax))
    {
        ...
        if ((std::isnan(rmax) || std::isinf(rmax)) && scaled)
        {
            ...
            // already rescaled
            double rfact = Alg1010Solver::CUBIC_RESCAL_FACT;
...
double Alg1010Solver::calc_err_ldlt(
...
double Alg1010Solver::calc_err_abcd_cmplx(double a,
                                          double b,
                                          double c,
                                          double d,
                                          std::complex<double> aq,
...
double Alg1010Solver::calc_err_abcd(double a,
                                    double b,
                                    double c,
                                    double d,
                                    double aq,
...
double Alg1010Solver::calc_err_abc(
...
void Alg1010Solver::NRabcd(double a,
                           double b,
                           double c,
                           double d,
                           double* AQ,
...
void Alg1010Solver::solve_quadratic(double a, double b, Comp2& roots)
{
...
// void Alg1010Solver::quartic_solver(double coeff[5], std::complex<double>
// roots[4])
{
    ...
    int nsol = 0;
    double bl311 = 2. * b / 3. - phi0 - l1 * l1; /* This is d2 as defined in
                                                    eq. (20)*/
    double dml3l3 = d - l3 * l3; /* dml3l3 is d3 as defined in eq. (15) with
                                    d2=0 */
    ...
    if (del2 != 0)
    {
        ...
        if (l2m[nsol] != 0)
        {
            ...
            res[nsol]
                = oqs::calc_err_ldlt(b, c, d, d2m[nsol], l1, l2m[nsol], l3);
            nsol++;
        }
                   ...
                   * (std::abs(2. * b / 3.) + std::abs(phi0) + l1 * l1)))
    {
        ...
        if (d3 <= 0)
        {
            ...
            if (std::abs(dq1) < std::abs(bq1))
                ...
                bq1 = d / dq1;
            err1 = oqs::calc_err_abcd(a, b, c, d, aq1, bq1, cq1, dq1); /* eq.
                                                                          (68)
                                                                        */
        }
        else
